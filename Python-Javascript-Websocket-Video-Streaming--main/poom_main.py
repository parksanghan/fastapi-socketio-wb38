...
async def indexes(request:Request,
    ...
    return response
     ...
     # 처음진입점에서 이제 socketio 연동 및 전체 ui 제공
@app.get('/join2',response_class=HTMLResponse,name='join')
async def indexes(request:Request,
          room_id:Optional[str]=None,
          display_name:Optional[str]=None,
          mute_audio:Optional[str]=None,
...
async def upload_file(sid, file: UploadFile = File(...)):
    ...
    # 파일 확장자가 txt 또는 pdf인지 확인
        if file_ext == "txt": # txt 는 그대로 저장 
            filepath = os.path.join(directory,f'{sid}.txt')
            ...
            asyncio.create_task(handle_voice(sid, givenData))
        elif file_ext == "pptx":
            filepath = os.path.join(directory,f'{sid}.pptx')
            await save_uploaded_file(givenData,filepath)
...
async def chat_handler(sid,data):
    ...
    #     await sio.emit('GetAnswer_Add', data=myfile, to=sid)
...
if __name__ == '__main__':
    ...
    uvicorn.run(combined_asgi_app, host='127.0.0.1',
                 port=5000,
